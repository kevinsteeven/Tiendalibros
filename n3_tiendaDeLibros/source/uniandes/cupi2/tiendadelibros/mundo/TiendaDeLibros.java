...
public class TiendaDeLibros
{
    ...
    {	double preciolibro, precionuevo;
    	for(Libro libro : catalogo) 
    	{preciolibro=libro.darPrecioVenta();
    	for (int i = 0; i < lim_inf.length; i++)
    		{
    		if (libro.darPrecioVenta()  >= lim_inf[i] && libro.darPrecioVenta()  < lim_sup[i])
				{
					precionuevo = preciolibro*(1-descuentos[i]);
					libro.Actualizarprecio(precionuevo);
				}
    		}
    ...
    {
    Transaccion mejortransaccion=null;
    double precioventa =0, valortransaccion=0, valormejortransaccion =0;
    int cantidadvendida, cantidadmejortransaccion=0;
    String idmejortransaccion= new String(),fechamejortransaccion= new String(); 
 
    	for(Libro libro : catalogo) 
		{ 	for (int j=0; j < libro.darTransacciones().toArray().length; j++)
    			{precioventa=libro.darPrecioVenta();
    			cantidadvendida=libro.darTransacciones().get(j).darCantidad();
				valortransaccion = cantidadvendida*precioventa;
				if (valortransaccion > valormejortransaccion)
					{valormejortransaccion=valortransaccion;
					cantidadmejortransaccion=cantidadvendida;
					idmejortransaccion =libro.darTransacciones().get(j).darIdTransaccion();
					fechamejortransaccion= libro.darTransacciones().get(j).darFecha();
					}
    			}	
		mejortransaccion= new Transaccion(Transaccion.Tipo.VENTA,cantidadmejortransaccion,fechamejortransaccion,idmejortransaccion);
		}
    return mejortransaccion;
}}
